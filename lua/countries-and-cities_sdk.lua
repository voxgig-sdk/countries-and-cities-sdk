...
require("countries-and-cities_types")
...
local CountriesAndCitiesSDK = {}
CountriesAndCitiesSDK.__index = CountriesAndCitiesSDK
...
CountriesAndCitiesSDK._make_feature = _make_feature


function CountriesAndCitiesSDK.new(options)
  local self = setmetatable({}, CountriesAndCitiesSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function CountriesAndCitiesSDK:options_map()
...
function CountriesAndCitiesSDK:get_utility()
  return Utility.copy(self._utility)
end


function CountriesAndCitiesSDK:get_root_ctx()
  return self._rootctx
end


function CountriesAndCitiesSDK:prepare(fetchargs)
...
function CountriesAndCitiesSDK:direct(fetchargs)
...
function CountriesAndCitiesSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function CountriesAndCitiesSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "CountriesAndCitiesSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function CountriesAndCitiesSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function CountriesAndCitiesSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "CountriesAndCitiesSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function CountriesAndCitiesSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = CountriesAndCitiesSDK.new(sdkopts)
  sdk.mode = "test"
