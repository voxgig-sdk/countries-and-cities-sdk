...
[MCP](https://modelcontextprotocol.io) server exposing the CountriesAndCities SDK as
two agent tools — `countries-and-cities_list` and `countries-and-cities_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/countries-and-cities-mcp)
make build

# 2. Provide credentials via the environment
export COUNTRIES_AND_CITIES_APIKEY=sk_live_xxx
...
```jsonc
// countries-and-cities_list: first page of records
{ "entity": "city" }
{ "entity": "city", "query": { } }

// countries-and-cities_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/countries-and-cities-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user countries-and-cities \
     -- "$PWD"/dist/*/countries-and-cities-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `countries-and-cities_list` and `countries-and-cities_load` tools now appear
   in new sessions. Ask the agent to *"list city using countries-and-cities"*
   and it calls `countries-and-cities_list` with `{"entity":"city"}`.
...
```sh
export COUNTRIES_AND_CITIES_APIKEY=sk_live_xxx            # API key
export COUNTRIES_AND_CITIES_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `countries-and-cities_list` tool
...
### Call the `countries-and-cities_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `countries-and-cities_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `countries-and-cities_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `COUNTRIES_AND_CITIES_APIKEY` | API key sent with every request. |
| `COUNTRIES_AND_CITIES_BASE` | Optional override of the API base URL. |
