...
// CountriesAndCities SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = CountriesAndCitiesOmni::makeStructRunner(
    ...
    {
        return CountriesAndCitiesOmni::objmodel(CountriesAndCitiesOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return CountriesAndCitiesOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return CountriesAndCitiesOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(CountriesAndCitiesOmni::structfix($now, Struct::undef()))
                        !== json_encode(CountriesAndCitiesOmni::structfix($val, Struct::undef()))) {
