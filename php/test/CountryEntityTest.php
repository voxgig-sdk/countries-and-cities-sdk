...
class CountryEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = CountriesAndCitiesSDK::test($seed, null);
        $seen = iterator_to_array($base->Country(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = CountriesAndCitiesConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = CountriesAndCitiesSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set COUNTRIES_AND_CITIES_TEST_COUNTRY_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $country_ref01_data_result = $country_ref01_ent->create($country_ref01_data, null);
        $country_ref01_data = Helpers::to_map(is_object($country_ref01_data_result) && method_exists($country_ref01_data_result, 'data_get') ? $country_ref01_data_result->data_get() : $country_ref01_data_result);
...
function country_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("COUNTRIES_AND_CITIES_TEST_COUNTRY_ENTID");
