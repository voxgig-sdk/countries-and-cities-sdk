...
//    only - a typed CountriesAndCitiesContext there would make every ctx
...
final class CountriesAndCitiesOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new CountriesAndCitiesContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new CountriesAndCitiesSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new CountriesAndCitiesResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new CountriesAndCitiesError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new CountriesAndCitiesResponse($clean['response']);
    ...
    public static function err_from_map($m): ?CountriesAndCitiesError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new CountriesAndCitiesError(is_string($code) ? $code : '', $msg);
    }
