...
// CountriesAndCities SDK feature test
//
...
class FtCtrl extends CountriesAndCitiesControl
{
...
class FtClient
{
    ...
    private CountriesAndCitiesUtility $utility;

    public function __construct(CountriesAndCitiesUtility $utility, array $options)
    {
    ...
    public function get_utility(): CountriesAndCitiesUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (CountriesAndCitiesContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public CountriesAndCitiesContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (CountriesAndCitiesContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = CountriesAndCitiesConfig::make_feature($name);
    ...
    private static function buildUrl(CountriesAndCitiesSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof CountriesAndCitiesError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new CountriesAndCitiesResponse($fetched);
        }
    ...
    private function fail(CountriesAndCitiesContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (CountriesAndCitiesContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
