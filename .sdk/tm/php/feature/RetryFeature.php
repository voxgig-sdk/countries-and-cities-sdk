...
// CountriesAndCities SDK retry feature
...
class CountriesAndCitiesRetryFeature extends CountriesAndCitiesBaseFeature
{
    ...
    public function init(CountriesAndCitiesContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (CountriesAndCitiesContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(CountriesAndCitiesContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(CountriesAndCitiesContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
