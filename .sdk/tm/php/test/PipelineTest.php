...
// CountriesAndCities SDK pipeline test
//
...
{
    private static function utility(): CountriesAndCitiesUtility
    {
        return new CountriesAndCitiesUtility();
    }
    ...
    private static function ctx(array $over = []): CountriesAndCitiesContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new CountriesAndCitiesContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new CountriesAndCitiesOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof CountriesAndCitiesError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new CountriesAndCitiesOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = CountriesAndCitiesMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new CountriesAndCitiesOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = CountriesAndCitiesMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new CountriesAndCitiesOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = CountriesAndCitiesMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new CountriesAndCitiesSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = CountriesAndCitiesMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new CountriesAndCitiesResponse([]),
            'result' => new CountriesAndCitiesResult([]),
        ]);
        [, $err] = CountriesAndCitiesMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new CountriesAndCitiesSpec([]),
            'response' => null,
            'result' => new CountriesAndCitiesResult([]),
        ]);
        [, $err] = CountriesAndCitiesMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new CountriesAndCitiesSpec(['step' => 's']),
            'response' => new CountriesAndCitiesResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new CountriesAndCitiesResult([]),
        ]);
        [, $err] = CountriesAndCitiesMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new CountriesAndCitiesSpec(['step' => 's']),
            'response' => new CountriesAndCitiesResponse(self::resp(200, ['v' => 1])),
            'result' => new CountriesAndCitiesResult([]),
        ]);
        [, $err] = CountriesAndCitiesMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new CountriesAndCitiesSpec(['step' => 's']),
            'response' => new CountriesAndCitiesResponse(self::resp(200, ['v' => 2])),
            'result' => new CountriesAndCitiesResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        CountriesAndCitiesMakeResponse::call($ctx);
    ...
    {
        $preset = new CountriesAndCitiesResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new CountriesAndCitiesSpec([]),
            'response' => new CountriesAndCitiesResponse([]),
            'result' => new CountriesAndCitiesResult([]),
        ...
        [$got, $err] = CountriesAndCitiesMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new CountriesAndCitiesResult([])]);
        [, $err] = CountriesAndCitiesMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new CountriesAndCitiesSpec([]), 'result' => null]);
        [, $err] = CountriesAndCitiesMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new CountriesAndCitiesSpec(['step' => 's']),
            'result' => new CountriesAndCitiesResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new CountriesAndCitiesOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = CountriesAndCitiesMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new CountriesAndCitiesSpec(['step' => 's']),
            'result' => new CountriesAndCitiesResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new CountriesAndCitiesOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = CountriesAndCitiesMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new CountriesAndCitiesResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new CountriesAndCitiesSpec([]),
            'result' => new CountriesAndCitiesResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = CountriesAndCitiesMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = CountriesAndCitiesMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new CountriesAndCitiesError('boom', 'boom');
        $utility->fetcher = function (CountriesAndCitiesContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new CountriesAndCitiesSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = CountriesAndCitiesMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (CountriesAndCitiesContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = CountriesAndCitiesMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(CountriesAndCitiesResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        CountriesAndCitiesMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (CountriesAndCitiesContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new CountriesAndCitiesSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = CountriesAndCitiesMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new CountriesAndCitiesResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new CountriesAndCitiesSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = CountriesAndCitiesMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = CountriesAndCitiesMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = CountriesAndCitiesMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new CountriesAndCitiesResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, CountriesAndCitiesDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new CountriesAndCitiesResult(['ok' => false])]);
        $this->expectException(CountriesAndCitiesError::class);
        CountriesAndCitiesDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new CountriesAndCitiesResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', CountriesAndCitiesMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        CountriesAndCitiesMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new CountriesAndCitiesBaseFeature();
        $b = new CountriesAndCitiesBaseFeature();
        CountriesAndCitiesFeatureAdd::call($ctx, $a);
        CountriesAndCitiesFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): CountriesAndCitiesBaseFeature
    {
        $f = new CountriesAndCitiesBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        CountriesAndCitiesFeatureAdd::call($ctx, self::named_feature('a'));
        CountriesAndCitiesFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        CountriesAndCitiesFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        CountriesAndCitiesFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        CountriesAndCitiesFeatureAdd::call($ctx, $replace);
        ...
        CountriesAndCitiesFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): CountriesAndCitiesContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new CountriesAndCitiesSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = CountriesAndCitiesPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        CountriesAndCitiesPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new CountriesAndCitiesResponse(['headers' => null]),
            'result' => new CountriesAndCitiesResult([]),
        ]);
        CountriesAndCitiesResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        CountriesAndCitiesResultBody::call($ctx);
