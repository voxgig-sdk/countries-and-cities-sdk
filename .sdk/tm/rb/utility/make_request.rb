...
module CountriesAndCitiesUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = CountriesAndCitiesResponse.new({})
    ...
    if fetch_err
      ...
      response = CountriesAndCitiesResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = CountriesAndCitiesResponse.new(fetched)
    else
