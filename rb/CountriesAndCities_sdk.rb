...
require_relative 'CountriesAndCities_types'
...
class CountriesAndCitiesSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = CountriesAndCitiesHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = CountriesAndCitiesHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, CountriesAndCitiesFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    CountriesAndCitiesUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = CountriesAndCitiesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = CountriesAndCitiesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = CountriesAndCitiesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => CountriesAndCitiesError.new(
        "#{op}_allow",
        "CountriesAndCitiesSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue CountriesAndCitiesError => err
    ...
    if fetched.is_a?(Hash)
      status = CountriesAndCitiesHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = CountriesAndCitiesError.new(
        "graphql_error", "CountriesAndCitiesSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = CountriesAndCitiesSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
