...
class CountryEntity
  ...
  def data_set(args)
    if args
      @_data = CountriesAndCitiesHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = CountriesAndCitiesHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Array<Country>, Array] the matching Country items; raises CountriesAndCitiesError on failure
  ...
  #
  # @param reqdata [CountryCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [Country, Hash] the created Country; raises CountriesAndCitiesError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        if ctx.result.resdata
          @_data = CountriesAndCitiesHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
