...
import (
	...
	omni "github.com/voxgig-sdk/countries-and-cities-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.CountriesAndCitiesError); is {
				out := map[string]any{
					"name":    "CountriesAndCitiesError",
...
func omniCtx(arg any, client *sdk.CountriesAndCitiesSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.CountriesAndCitiesSDK); is {
			client = live
