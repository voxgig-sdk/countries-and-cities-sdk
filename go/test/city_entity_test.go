...
func TestCityEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		cityRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "city"}), "city_ref01"))

		cityRef01DataResult, err := cityRef01Ent.Create(cityRef01Data, nil)
		if err != nil {
			t.Fatalf("create failed: %v", err)
		}
		cityRef01Data = core.ToMapAny(entityData(cityRef01DataResult))
...
func cityBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("COUNTRIES_AND_CITIES_TEST_CITY_ENTID")
	...
	idmapResolved := core.ToMapAny(env["COUNTRIES_AND_CITIES_TEST_CITY_ENTID"])
