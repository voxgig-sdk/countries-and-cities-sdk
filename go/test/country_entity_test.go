...
func TestCountryEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set COUNTRIES_AND_CITIES_TEST_COUNTRY_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		countryRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "country"}), "country_ref01"))

		countryRef01DataResult, err := countryRef01Ent.Create(countryRef01Data, nil)
		if err != nil {
			t.Fatalf("create failed: %v", err)
		}
		countryRef01Data = core.ToMapAny(entityData(countryRef01DataResult))
...
func countryBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("COUNTRIES_AND_CITIES_TEST_COUNTRY_ENTID")
	...
	idmapResolved := core.ToMapAny(env["COUNTRIES_AND_CITIES_TEST_COUNTRY_ENTID"])
