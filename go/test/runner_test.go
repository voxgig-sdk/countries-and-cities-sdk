...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("COUNTRIES_AND_CITIES_TEST_LIVE") == "TRUE" ||
		os.Getenv("COUNTRIES_AND_CITIES_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("COUNTRIES_AND_CITIES_TEST_EXPLAIN"); explain != "" {
		m["COUNTRIES_AND_CITIES_TEST_EXPLAIN"] = explain
	}
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.CountriesAndCitiesSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.CountriesAndCitiesError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.CountriesAndCitiesSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.CountriesAndCitiesError{Msg: msg, Code: code}
}
