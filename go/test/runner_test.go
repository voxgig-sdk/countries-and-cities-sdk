...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("COUNTRIESANDCITIES_TEST_LIVE") == "TRUE" ||
		os.Getenv("COUNTRIESANDCITIES_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("COUNTRIESANDCITIES_TEST_EXPLAIN"); explain != "" {
