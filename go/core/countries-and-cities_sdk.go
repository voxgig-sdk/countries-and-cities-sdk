...
type CountriesAndCitiesSDK struct {
	Mode     string
...
func (sdk *CountriesAndCitiesSDK) OptionsMap() map[string]any {
...
func (sdk *CountriesAndCitiesSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *CountriesAndCitiesSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *CountriesAndCitiesSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *CountriesAndCitiesSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *CountriesAndCitiesSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath(sdk.options, []any{"allow", "op"}).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *CountriesAndCitiesSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath(sdk.options, []any{"allow", "op"}).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("CountriesAndCitiesSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *CountriesAndCitiesSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *CountriesAndCitiesSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("CountriesAndCitiesSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
...
func TestSDK(testopts map[string]any, sdkopts map[string]any) *CountriesAndCitiesSDK {
	if sdkopts == nil {
	...
	sdk := NewCountriesAndCitiesSDK(sdkopts)
	sdk.Mode = "test"
