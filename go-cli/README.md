# countries-and-cities-cli

boru-driven command-line client **and** interactive REPL for the CountriesAndCities
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/countries-and-cities-cli)
make build

# 2. See usage (words, entities, env vars)
./countries-and-cities-cli --help

# 3. Provide credentials once, via the environment
export COUNTRIES_AND_CITIES_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./countries-and-cities-cli list city
./countries-and-cities-cli list country

# 5. Override the API base URL for a single call
COUNTRIES_AND_CITIES_BASE=https://api.example.com ./countries-and-cities-cli list city

# 6. No arguments -> interactive REPL
./countries-and-cities-cli
countries-and-cities> list city
countries-and-cities> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/countries-and-cities-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/countries-and-cities-cli list city
   ```
...
Run with no arguments to open a REPL (prompt `countries-and-cities>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/countries-and-cities-cli`. |
...
(`./countries-and-cities-cli <expr>`) and interactively (the REPL), and expressions compose the
