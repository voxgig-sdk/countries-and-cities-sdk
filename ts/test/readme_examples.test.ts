...
//     (\`new CountriesAndCitiesSDK(...)\` / \`CountriesAndCitiesSDK.test(...)\` become
//     \`CountriesAndCitiesSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'CountriesAndCitiesSDK'
...
const TEST_SEED = {"entity":{"city":{"test01":{"id":"test01"}},"country":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new CountriesAndCitiesSDK(...)\` and any
// \`CountriesAndCitiesSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+CountriesAndCitiesSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/CountriesAndCitiesSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
