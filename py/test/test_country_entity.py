...
class TestCountryEntity:
    ...
    def test_should_stream(self):
        ...
        base = CountriesAndCitiesSDK.test(seed, None)
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = CountriesAndCitiesSDK.test(
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set COUNTRIES_AND_CITIES_TEST_COUNTRY_ENTID JSON to run live")
        client = setup["client"]
        ...
        country_ref01_data = helpers.to_map(runner.entity_data(country_ref01_ent.create(country_ref01_data, None)))
...
def _country_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("COUNTRIES_AND_CITIES_TEST_COUNTRY_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("COUNTRIES_AND_CITIES_TEST_LIVE") == "TRUE":
    ...
    _live = env.get("COUNTRIES_AND_CITIES_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("COUNTRIES_AND_CITIES_TEST_EXPLAIN") == "TRUE",
        "live": _live,
