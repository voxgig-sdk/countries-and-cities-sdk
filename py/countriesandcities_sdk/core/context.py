...
from countriesandcities_sdk.core.helpers import get_ctx_prop, to_map
...
class CountriesAndCitiesContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = CountriesAndCitiesControl()
        ...
        if isinstance(sp, CountriesAndCitiesSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, CountriesAndCitiesResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, CountriesAndCitiesResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return CountriesAndCitiesOperation({})
        ...
        op = CountriesAndCitiesOperation({
            "entity": entname,
            "name": opname,
            "input": inpt,
            "points": points,
    ...
    def make_error(self, code, msg):
        return CountriesAndCitiesError(code, msg, self)
